...
public class SecondProg_lab {
  ...
  static void printBookData(Book b) {
    String str =
      "Book has " + b.title + "title " + "whose author is " + b.author;
  ...
  static boolean isSubString(String s1, String s2) {
    int M = s1.length();
    int N = s2.length();/* A loop to slide pat[] one by one */
    for (int i = 0; i <= N - M; i++) {
      int j;
 ...
 pattern match */
  ...
  public static void main(String args[]) {
    ...
    System.out.println(
      "To list all books with price greater than a value, enter a price"
    );
